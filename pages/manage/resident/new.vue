<template>
	<div>
      ...
      <div class="d-flex align-center">
        <h2 class="mt-2 ml-4">Add new resident</h2>
      </div>
  ...
  export default {
    ...
    computed: {
      ...
      emailErrors () {
        ...
        !this.$v.residentData.email.email && errors.push('Must be valid e-mail')
        return errors
      },
      cstreetErrors () {
        const errors = []
      ...
      async submit() {
        ...
        if(!this.$v.$invalid) {
          ...
          await this.$store.dispatch('admin/newResident', formData).then(() => {
