	<template>
		<div>
	      ...
	      <div class="d-flex align-center">
	        <h2 class="mt-2">Edit official information</h2>
	        <v-btn rounded
	          color="primary"
	          class="ml-auto"
	          nuxt
	                  ...
	                  <v-row>
	                    ...
	                    <!-- <v-col align-self="start"cols="12" md="3">
	                      ...
	                      <v-avatar
	                        ...
	                        tile
	                      >
	                        <img id="previewSignature" :src="photoSrc1" alt="">
	                      </v-avatar>
	    ...
	    computed: {
	      ...
	      async fetchSingleOfficial(id) {
	        await this.$api.post(`singleOfficial/${id}`).then((res) => {
	          ...
	          this.official.id = official.id,
	          this.official.firstname = official.firstname,
	          this.official.middlename = official.middlename,
	          this.official.lastname = official.lastname,
	          this.official.gender = official.gender,
	          this.official.bdate = official.birthdate,
	          this.official.phone = official.phone,
	          this.official.position = official.position,
	          this.official.email = official.email,
	          this.official.photo = official.photo
	          this.official.signature = official.signature

	          this.photoText = official.photo != '' ? 'Change image' : 'Choose image'
	          this.photoText1 = official.signature != '' ? 'Change image' : 'Choose image'
	          this.photoSrc = official.photo && official.photo != 'null'? `${this.$config.laraURL}/officials/${official.photo}` : `${this.$config.laraURL}/accounts/nopreview.png`
	          this.photoSrc1 = official.signature && official.signature != 'null'? `${this.$config.laraURL}/officials/${official.signature}` : `${this.$config.laraURL}/accounts/nopreview.png`
	        })
	      },
	      async submit() {
	        this.$v.$touch();
	        if(!this.$v.$invalid) {
	          ...
	          await this.$store.dispatch('admin/updateOfficial', formData)
